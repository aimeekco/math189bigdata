...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		ind = np.random.randint(0, m, batch_size)
		W_grad = find_grad(X[ind], y[ind], W, reg)
		W = prox(W - lr * W_grad, reg * lr)
		cost = find_cost(X[ind], y[ind], W, reg)
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
	...
	for index in range(num_reg):
		...
		reg = 1. / tau_list[index]
		W[:, index] = grad_lasso(X, y, reg, lr=1e-12, max_iter=1000, batch_size=1024, print_freq=1000)[0].flatten()
