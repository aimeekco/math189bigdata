...
def linreg(X, y, reg=0.0):
	...
	"""
	"*** YOUR CODE HERE ***"
	identitym = np.eye(X.shape[1])
	identitym[0,0] = 0
	W_opt = np.linalg.solve(X.T @ X + reg * identitym, X.T @ y)
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	difference = y - predict_y
	m = X.shape[0]
	RMSE = np.sqrt(np.linalg.norm(difference, 2) ** 2 / m)
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	"*** YOUR CODE HERE ***"
	opt_lambda_ind = np.argmin(RMSE_list)
	reg_opt = reg_list[opt_lambda_ind]
	"*** END YOUR CODE HERE ***"
	return reg_opt
